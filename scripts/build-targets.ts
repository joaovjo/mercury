 ...
 *
 * Assumes the web assets are already built and embedded (run `bun run build:web`
 * and `bun run embed` first). Produces dist/mercury-<os>-<arch> plus a SHA256SUMS file.
 *
...
const entry = join(rootDir, "apps/cli/src/index.ts");
const sums: string[] = [];
...
for (const { bunTarget, asset } of TARGETS) {
	...
	sums.push(`${sha}  ${asset}`);
	console.log(
		`  ✓ ${asset} (${(buf.length / 1e6).toFixed(0)} MB)  ${sha.slice(0, 12)}…`,
	);
}
