...
import type { Channel, OutreachState } from "@mercury/outreach";
import {
	blockedForCompany,
	checkBlocked,
	dueAttempts,
	...
	setBudget,
	transition,
} from "@mercury/outreach";
import { loadOutreachConfig } from "@mercury/core";
import { type Flags, int, reqStr, str } from "./flags.ts";

function fmtAttempt(a: OutreachAttempt): string {
	const who = a.person_name
		? `${a.person_name} (${a.person_username})`
		: a.person_username;
	const co = a.company_name ?? a.company_urn;
	const due = a.next_action_due
		? ` · due ${a.next_action_due.slice(0, 10)}`
		: "";
	...
	switch (sub) {
		...
		case "due": {
			...
			for (const { attempt, action } of items) {
				console.log(
					`[${action.kind.toUpperCase()}] ${fmtAttempt(attempt)} — ${action.reason}`,
				);
			}
			return;
		}
		...
		case "list": {
			...
			const rows = db()
				.query(
					`SELECT * FROM outreach_attempts ${where} ORDER BY updated_at DESC`,
				)
		...
		case "blocked": {
			...
			for (const a of rows) {
				const until = a.block_until
					? ` (until ${a.block_until.slice(0, 10)})`
					: " (permanent)";
				console.log(
					`  ${a.person_name ?? a.person_username}: ${a.state}${until} — ${a.reason ?? ""}`,
				);
			}
			return;
		}
		...
		case "budget": {
			...
			if (int(flags, "remaining") !== undefined)
				patch.credits_remaining = int(flags, "remaining")!;
			...
			if (int(flags, "reset-day") !== undefined)
				patch.cycle_reset_day = int(flags, "reset-day")!;
		...
		case "withdraw": {
			...
			const { withdrawInvitation } = await import("@mercury/mcp");
			const a = db()
				.query("SELECT * FROM outreach_attempts WHERE id = ?")
				.get(id) as OutreachAttempt | undefined;
			if (!a) {
			...
			await notifyOutreach();
			console.log(
				`outreach #${id} → invite_ignored (${withdrawn ? "withdrawn" : "degraded"})`,
			);
			return;
		}
