...
export async function recruiterCmd(sub: string, flags: Flags): Promise<void> {
	...
	if (sub === "update") {
		...
		if (status) {
			...
			if (status === "replied")
				sets.push("replied_at = COALESCE(replied_at, datetime('now'))");
		}
		...
		d.query(`UPDATE recruiters SET ${sets.join(", ")} WHERE id = $id`).run(
			params,
		);
		await notifyChange("recruiters");
...
async function recruiterSyncCmd(flags: Flags): Promise<void> {
	const { runSync } = await import("@mercury/recruiter");
	...
	try {
		...
		console.error(
			`error: recruiter sync failed — ${err instanceof Error ? err.message : err}`,
		);
		process.exit(1);
	}
	...
	if (!result.changes.length) {
		...
		for (const c of result.changes) {
			console.log(
				`    #${c.id} ${c.name} @ ${c.company ?? "?"} — ${c.from} → ${c.to} (by ${c.matchedBy})`,
			);
		}
	}
