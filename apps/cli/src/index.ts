#!/usr/bin/env bun
import { checkForUpdate, maybePrintUpdateNotice, VERSION } from "@mercury/core";
...
Usage:
  ...
  mercury tui                        Launch interactive terminal UI
...
async function main() {
	...
	switch (cmd) {
		...
		case "setup":
			...
			break;
		case "dashboard": {
			const { startDashboardServer } = await import("@mercury/server");
		...
		case "tui": {
			const { tuiCmd } = await import("./tui.ts");
			await tuiCmd(flags);
			return;
		}
		case "import-journey": {
