...
export async function applicationCmd(sub: string, flags: Flags): Promise<void> {
	...
	if (sub === "update") {
		...
		if (sets.length === 0) {
			console.error(
				"error: nothing to update (pass --status/--portal/--external-url/--fields/--unfilled/...)",
			);
			process.exit(1);
		}
		const res = d
...
export async function answerCmd(sub: string, flags: Flags): Promise<void> {
	...
	if (sub === "list") {
		...
		const rows = cat
			? d
					.query(
						`SELECT key, value, category, updated_at FROM applicant_answers WHERE category = ? ORDER BY category, key`,
					)
					.all(cat)
			: d
					.query(
						`SELECT key, value, category, updated_at FROM applicant_answers ORDER BY category, key`,
					)
					.all();
