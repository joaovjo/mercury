...
function detectAsset(): string {
	...
	if (os === "windows") {
		if (arch !== "x64")
			throw new Error("Windows only supports x64 prebuilt binaries");
...
async function copySkills(sourceDir: string): Promise<void> {
	...
	if (Bun.env.MERCURY_SKILLS_DIR) {
		...
		if (await pathExists(opencodeDir))
			targets.push(join(opencodeDir, "skills"));
		if (await pathExists(claudeDir)) targets.push(join(claudeDir, "skills"));
	}

	if (targets.length === 0) {
		warn(
			"No agent skills dir detected — set MERCURY_SKILLS_DIR to copy skills.",
		);
		return;
	}
...
async function installFromSource(): Promise<void> {
	...
	if (!Bun.which("bun")) {
		...
		if (installResult.exitCode !== 0)
			die(
				"Bun install failed. Install manually from https://bun.sh and re-run.",
			);
		const bunInstall = Bun.env.BUN_INSTALL ?? join(HOME, ".bun");
		process.env.PATH = `${join(bunInstall, "bin")}:${process.env.PATH}`;
		if (!Bun.which("bun"))
			die("Bun installed but not on PATH. Restart your shell and re-run.");
		ok("Bun installed");
	} else {
	...
	if (await pathExists(join(SRC_DIR, ".git"))) {
		...
		const cloneResult =
			await $`git clone --depth 1 --branch ${REF} ${REPO} ${SRC_DIR} --quiet`.nothrow();
	...
	if (!(await pathExists(binaryPath)))
		die(`Build did not produce dist/mercury${EXE}`);
	...
	try {
		...
		if (Bun.env.MERCURY_NO_SKILLS !== "1") {
			...
			const tgzResp = await fetch(
				`https://codeload.github.com/${REPO_SLUG}/tar.gz/refs/tags/${tag}`,
			).catch(() => null);
			if (tgzResp?.ok) {
				...
				warn(
					"Could not fetch skills tarball; re-run with MERCURY_FROM_SOURCE=1 to copy skills.",
				);
			}
		}
...
async function mkdtemp(): Promise<string> {
	if (IS_WINDOWS) {
		...
		const dir = join(
			tmpBase,
			`mercury-install-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
