...
describe("detectPortal", () => {
	test("greenhouse by host", () => {
		expect(
			detectPortal("https://job-boards.greenhouse.io/gitlab/jobs/8497793002")
				.portal,
		).toBe("greenhouse");
		expect(detectPortal("https://boards.greenhouse.io/acme").portal).toBe(
			"greenhouse",
		);
	});
	...
	test("ashby by host", () => {
		expect(
			detectPortal("https://jobs.ashbyhq.com/ashby/ce9f0432/application")
				.portal,
		).toBe("ashby");
	});
