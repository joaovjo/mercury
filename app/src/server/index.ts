...
import { listProviderModels, PROVIDERS } from "../acp/providers.ts";
...
import { ensureHome, loadConfig, paths } from "../paths.ts";
...
async function handleSearch(path: string, req: Request): Promise<Response> {
	...
	try {
		const { searchJobs, searchPeople, jobDetails } = await import(
			"../mcp/search.ts"
		);
		switch (path) {
