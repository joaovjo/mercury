...
export function findSkillsSource(): string | null {
	...
	if (process.env.MERCURY_SKILLS_SRC)
		candidates.push(process.env.MERCURY_SKILLS_SRC);
	...
	for (const c of candidates) {
		if (c && existsSync(c) && existsSync(join(c, "job-scout", "SKILL.md")))
			return c;
	}
	return null;
}
...
async function downloadSkills(): Promise<string | null> {
	...
	if (existsSync(join(cachedSkills, "job-scout", "SKILL.md")))
		return cachedSkills;
	...
	for (const ref of refs) {
		try {
			...
			const top = readdirSync(tmp).find(
				(d) => d.startsWith("mercury-") || d.includes("-mercury-"),
			);
			const extractedSkills = top ? join(tmp, top, "skills") : null;
			if (
				extractedSkills &&
				existsSync(join(extractedSkills, "job-scout", "SKILL.md"))
			) {
