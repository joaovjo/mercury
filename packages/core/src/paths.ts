...
export function getMercuryHome(): string {
	return process.env.MERCURY_HOME ?? join(homedir(), ".mercury");
}

export const MERCURY_HOME = getMercuryHome();
...
export const paths = {
	...
	get db(): string {
		return join(getMercuryHome(), "mercury.db");
	},
	get config(): string {
		return join(getMercuryHome(), "config.json");
	},
	/** Lockfile written by a running dashboard so the CLI can notify it of DB changes. */
	get serverLock(): string {
		return join(getMercuryHome(), "dashboard.lock");
	},
	...
	get updateCache(): string {
		return join(getMercuryHome(), "update-check.json");
	},
};
