...
import { DEFAULT_OUTREACH_CONFIG } from "@mercury/core";
import {
	type AttemptLike,
	addDays,
	addMonths,
...
describe("canTransition — legal transitions", () => {
	...
	test("accepted → engaged", () =>
		expect(canTransition("accepted", "engaged")).toBe(true));
...
describe("canTransition — illegal transitions", () => {
	...
	test("engaged → accepted", () =>
		expect(canTransition("engaged", "accepted")).toBe(false));
...
describe("dueAction", () => {
	...
	test("invited: null sent_at → no action (NaN guard)", () => {
		expect(
			dueAction(attempt("invited", { sent_at: null }), cfg, BASE),
		).toBeNull();
	});
	...
	test("accepted: no action before threshold (3 days)", () => {
		...
		const now = BASE;
		expect(
			dueAction(attempt("accepted", { first_msg_at }), cfg, now),
		).toBeNull();
	});
	...
	test("accepted: null first_msg_at → no action", () => {
		expect(
			dueAction(attempt("accepted", { first_msg_at: null }), cfg, BASE),
		).toBeNull();
	});
	...
	test("followed_up: no action before threshold (6 days)", () => {
		...
		const now = BASE;
		expect(
			dueAction(attempt("followed_up", { followed_up_at }), cfg, now),
		).toBeNull();
	});
	...
	test("followed_up: close due exactly at threshold (7 days)", () => {
		...
		const action = dueAction(
			attempt("followed_up", { followed_up_at }),
			cfg,
			now,
		);
...
describe("nextActionDue", () => {
	...
	test("invited: null sent_at → null", () => {
		expect(
			nextActionDue(attempt("invited", { sent_at: null }), cfg),
		).toBeNull();
	});
	...
	test("accepted: due = first_msg_at + followupAfterDays (4)", () => {
		...
		expect(nextActionDue(attempt("accepted", { first_msg_at }), cfg)).toBe(
			expected,
		);
	});
	...
	test("accepted: null first_msg_at → null", () => {
		expect(
			nextActionDue(attempt("accepted", { first_msg_at: null }), cfg),
		).toBeNull();
	});
	...
	test("followed_up: due = followed_up_at + followupGraceDays (7)", () => {
		...
		const expected = addDays(BASE, 7);
		expect(nextActionDue(attempt("followed_up", { followed_up_at }), cfg)).toBe(
			expected,
		);
	});
	...
	test("followed_up: null followed_up_at → null", () => {
		expect(
			nextActionDue(attempt("followed_up", { followed_up_at: null }), cfg),
		).toBeNull();
	});
...
describe("blockUntil", () => {
	...
	test("permanent block → null", () => {
		const cfg = {
			...
			companyBlock: "permanent" as const,
		};
