...
import { db, now, notifyChange } from "@mercury/db";
import { loadOutreachConfig, type OutreachConfig } from "@mercury/core";
import {
	type AttemptLike,
	BLOCKING_STATES,
	blockUntil,
...
export function transition(
	...
	if (!canTransition(a.state, to))
		throw new Error(
			`illegal outreach transition: ${a.state} → ${to} (attempt #${id})`,
		);
	...
	if (to === "engaged" && a.cost_credits === 1 && a.credit_refunded === 0) {
		if (
			...
			new Date(nowIso) <= new Date(a.refund_eligible_until)
		) {
	...
	return d
		.query(
			`UPDATE outreach_attempts SET ${sets.join(", ")} WHERE id = $id RETURNING *`,
		)
...
export function getBudget(d: Database = db()): BudgetRow {
	return d
		.query("SELECT * FROM outreach_budget WHERE id = 1")
		.get() as BudgetRow;
}
...
export function setBudget(
	...
	if (cols.length) {
		...
		d.query(
			`UPDATE outreach_budget SET ${sets}, last_synced = $now WHERE id = 1`,
