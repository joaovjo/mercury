 ...
 */
import type { OutreachConfig } from "@mercury/core";
...
export function decideChannel(opts: {
	...
	const { degree, openProfile, highValue, creditsRemaining, reserveFloor } =
		opts;
	...
	if (degree === "1st")
		return {
			channel: "message",
			costCredits: 0,
			rationale: "1st-degree: free direct message",
		};
	...
	if (openProfile)
		return {
			channel: "inmail",
			costCredits: 0,
			rationale: "Open Profile: free InMail",
		};
	...
	return {
		...
		costCredits: 0,
		rationale:
			"3rd-degree, not high-value: seek a mutual-connection intro instead of spending a credit",
	};
}
...
export function nextActionDue(
	...
	switch (a.state) {
		...
		case "accepted":
			return a.first_msg_at
				? addDays(a.first_msg_at, cfg.followupAfterDays)
				: null;
		case "followed_up":
			return a.followed_up_at
				? addDays(a.followed_up_at, cfg.followupGraceDays)
				: null;
		default:
			return null;
	}
