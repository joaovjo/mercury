...
import { dueAttempts, getBudget } from "@mercury/outreach";
...
export const queries = {
	...
	outreach() {
		...
		const funnelRows = d
			.query(
				"SELECT state, COUNT(*) AS n FROM outreach_attempts GROUP BY state",
			)
		...
		const blockedMap = new Map<
			string,
			{ company_name: string | null; company_urn: string; count: number }
		>();
		for (const r of blockingRows) {
			const active =
				!r.block_until || new Date(r.block_until) > new Date(nowIso);
			if (!active) continue;
